...
The replay viewer is a separate executable shipped beside Kumori in a published build. It is compiled against the official [ppy/osu](https://github.com/ppy/osu) source tree and osu.Framework. A source build obtains that dependency in `third_party\osu\`; the checkout is intentionally excluded from Git because it is upstream code and large.
