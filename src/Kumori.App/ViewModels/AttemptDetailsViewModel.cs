...
public partial class AttemptDetailsViewModel : ObservableObject
{
    ...
    public bool HasLoadError => Details is null && !string.IsNullOrWhiteSpace(LoadError);
    public bool ShowEmptyState => Details is null && !IsLoading && !HasLoadError;
    ...
    public IReadOnlyList<ModEntry> DisplayMods
    {
        get
        {
            ...
            var mods = details.Mods.Count > 0
                ? details.Mods
                : ModDisplayText.AcronymsFromKey(details.Summary.ModsKey)
                    .Select(acronym => new ModEntry(acronym, "{}"))
                    .ToArray();
            return ModDisplayOrder.Sort(mods);
        }
    }
    ...
    partial void OnDetailsChanged(AttemptDetails? value)
    {
        ...
        OnPropertyChanged(nameof(HasDisplayMods));
