...
public sealed class ProcessMemory : IDisposable
{
    ...
    private ProcessMemory(SafeProcessHandle handle)
    {
        _handle = handle;
    }
    ...
    {
        if (!ReadProcessMemory(_handle, address, _scalarBuffer, (nuint)count, out var bytesRead)
            || bytesRead != (nuint)count)
            throw new Win32Exception(Marshal.GetLastWin32Error());
    }
    ...
    public nint FindPointer(nint baseAddress, long size, nint value)
    {
        ...
        while (remaining > 0)
        {
            ...
            if (search.TrySearch(address, buffer, out var match))
                return match;
